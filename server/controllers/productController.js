...
export const createProduct = async (req, res) => {
  const { title, image, description, rating, price, category, duration } =
    req.body;
  if (
    !title ||
    !image ||
...
export const searchProduct = async (req, res) => {
  try {
    ...
    if (!query) {
      res
        .status(400)
        .json({ success: false, errMsg: "Search queryy is required" });
      return;
    }
    const products = await PRODUCT.find({
      $or: [
    ...
    if (!products || products.length === 0) {
      res
        .status(404)
        .json({ success: false, errMsg: "No title or category found" });
      return;
    }
